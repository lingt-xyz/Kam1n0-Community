...
public class LearnerAsm2VecNew implements Serializable {

	private static final long serialVersionUID = -743890181829256933L;

	private static Logger logger = LoggerFactory.getLogger(LearnerAsm2VecNew.class);
	...
	public volatile transient StageInfo stage;
	...
	private void gradientDecend(final Iterable<FuncTokenized> funcs, Map<String, NodeWord> funcMap, long numTkns,
			long alphaUpdateInterval, boolean updateWordVec, int iterations, int parallelism)
			throws InterruptedException, ExecutionException {
		tknLastUpdate = 0;
		tknCurrent = 0;
		// training
		GradientProgress p = new GradientProgress(numTkns * iterations);
		if (debug)
			p.start(logger);

		// thread-safe batch consumer generator:
		final IteratorSafeGen<FuncTokenized> gen = new IteratorSafeGen<>(funcs, 100, iterations, iterationHood, ite -> {
			this.iteration = ite;
			// logger.info("Iteration {}", ite);
		});

		new Pool(parallelism).start(indx -> {
			...
			gen.subIterable().forEach(func -> {
				...
				if (tknCurrent - tknLastUpdate > alphaUpdateInterval) {
					...
					if (stage != null) {
						stage.progress = tknCurrent * 1.0 / p.totalCount;
					}
	...
	public void train(Iterable<FuncTokenized> funcs) throws InterruptedException, ExecutionException {
	...
	public Map<String, double[]> infer(List<FuncTokenized> funcs) {
		...
		try {
			...
			gradientDecend(funcs, inferDocMap, tknTotalInDocs, 0, false, param.optm_iteration / 2, 1);
	...
	public LearnerAsm2VecNew(Asm2VecNewParam param) {
		this.param = param;
	}
}
